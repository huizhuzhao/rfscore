...
def sybyl_atom_type_descriptor(protein, ligand, binsize=0.0):
    ...
    if binsize: 
        sum_descriptor_bins(descriptor, bins)
...
def sift_descriptor(protein, ligand, binsize=0.0):
    ...
    for hetatm, hetatm_contacts in contacts:
	    ...
	    sift = numpy.zeros(descriptor.size, dtype=np.int32)
            residue = atom.GetResidue()
            ...
            if interactions.is_hbond(hetatm,hetatm_types,atom,atom_types,distance): 
                sift[4 * offset + bin_id] = 1
            if interactions.is_weak_hbond(hetatm,hetatm_types,atom,atom_types,distance): 
                sift[5 * offset + bin_id] = 1
            if interactions.is_xbond(hetatm,hetatm_types,atom,atom_types,distance): 
                sift[6 * offset + bin_id] = 1
            if interactions.is_ionic(hetatm,hetatm_types,atom,atom_types,distance): 
                sift[7 * offset + bin_id] = 1
            if interactions.is_metal_complex(hetatm,hetatm_types,atom,atom_types,distance): 
                sift[8 * offset + bin_id] = 1
            if interactions.is_aromatic(hetatm,hetatm_types,atom,atom_types,distance): 
                sift[9 * offset + bin_id] = 1
            if interactions.is_hydrophobic(hetatm,hetatm_types,atom,atom_types,distance): 
                sift[10 * offset + bin_id] = 1
            if interactions.is_carbonyl(hetatm,hetatm_types,atom,atom_types,distance): 
